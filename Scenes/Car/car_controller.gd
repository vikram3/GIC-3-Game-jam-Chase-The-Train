...
func _process(delta):
	...
	if RaceManager.is_countdown_finished and !is_race_finished:
		...
		#distance_to_next_waypoint = snapped(distance_to_next_waypoint, 2)
		...
		print(car_name, " pos", " ", race_position)
...
func _physics_process(delta):
	...
	if is_AI and RaceManager.is_countdown_finished:
		...
		#var angle_to_target = atan2(target_direction.x, target_direction.z) - atan2(model.global_transform.basis[0].x, model.global_transform.basis[0].z) 

		# Adjust the car's steering based on the cross product
		if cross.y > 0.1:
			model_rotation.y += 200 * delta  # Turn right
		elif cross.y < -0.1:
			model_rotation.y -= 200 * delta  # Turn left
...
func _on_area_3d_area_entered(area):
	# Calculate lap time, best lap time, and lap number
	if area.is_in_group("finish") and checked == waypoints.size():
				
		if current_lap >= RaceManager.total_laps:
			# finish race
			lap_times.append(current_lap_time)
			race_time += current_lap_time
			check_best_lap()
		...
		# Lap completed, then reset current lap time and waypoints checked
		current_lap_time = 0
		checked = 0
		
	elif area.is_in_group("check"):
		#print(checked, current_waypoint)
